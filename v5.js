...
(() => {
  ...
  if (meta) meta.content = 'Local AI Foundry v5.2。RI#4 v0.64 / RI#5 v0.99のCurrent CandidateをPublic-safeに反映したHuman-Directed AI Foundry。';
  document.title = 'Local AI Foundry v5.2 — RI#4 v0.64 / RI#5 v0.99';
  ...
  const pulseViews = {
    overview: { title:'OVERVIEW', sub:'FOUNDRY v5.2 / CROSS-RI', metrics:[['PROJECT STATE','Published','info'],['CURRENT VECTOR','CORE EXTRACTION','core'],['FROZEN','RI#1','frozen'],['CORE CONTRIBUTORS','RI#2 / RI#3','ok'],['CURRENT CANDIDATE','RI#4 v0.64','frontier'],['CONTROL PLANE','RI#5 v0.99','info']] },
    ...
    ri4: { title:'RI#4', sub:'RESEARCH-GROUNDED LONG-FORM / v0.64', metrics:[['STRUCTURED SELECTION','BOUND','info'],['QUALITY GATE','BOUND','ok'],['SEMANTIC REVIEW','OPEN','frontier'],['ACCEPTANCE','NOT REACHED','frozen'],['CANDIDATE','v0.64','info'],['RUNTIME VERIFY','PENDING','core']] },
    ri5: { title:'RI#5', sub:'FOUNDRY CONTROL PLANE / v0.99', metrics:[['ROLE','CONTROL PLANE','info'],['LF-EKB','v0.3','ok'],['EVIDENCE EXPLORER','PHASE 1','core'],['HUMAN INPUT','BOUND','ok'],['RUNTIME VERIFY','PENDING','frontier'],['AUTHORITY','PRODUCER RI','info']] }
  };
  ...
  if (board && root) {
      ...
      ri4.innerHTML = `
        <div aria-label="RI#4 Research-Grounded Long-formの詳細を開く" class="ri4-rail-row" role="button" tabindex="0">
          <button aria-controls="ri4-telemetry" aria-expanded="false" class="line-toggle" id="ri4-toggle" type="button">
            <span class="line-id"><i>◎</i><b>RI#4</b></span><span class="line-name"><strong>Research Long-form</strong><small>v0.64 / Runtime Verification</small></span><span class="line-state">ACTIVE</span><span aria-hidden="true" class="chevron">⌄</span>
          </button>
          ...
          <div class="telemetry-reveal-inner"><section class="telemetry-panel ri4-telemetry-panel">
            ...
            <div class="telemetry-main">
              ...
              <div class="status-grid">
                <article class="status-card v5-frontier boot-item" style="--boot:2"><small>CURRENT CANDIDATE</small><strong>v0.64</strong></article>
              ...
              <div class="telemetry-lower">
                ...
                <div class="evidence-block boot-item" style="--boot:10"><p class="micro-title">EVIDENCE MODE</p><div aria-hidden="true" class="evidence-ring"><span>LOOP</span></div><ul><li><span>Quality Gate</span><strong>BOUND</strong></li><li><span>Current Candidate</span><strong>v0.64</strong></li><li><span>Acceptance</span><strong>NOT REACHED</strong></li></ul></div>
              </div>
              <footer class="telemetry-foot boot-item" style="--boot:11"><span>HUMAN RUNTIME VERIFICATION PENDING</span><p>Structured selectionとQuality GateをCurrent CandidateへBinding済み。残存Semantic / Editorial findingsを分離し、Production Acceptanceへ昇格しない。</p></footer>
            </div>
          </section></div>
        </div>`;
    ...
    const ri5 = promoteOperationalLine(ri5Line,'ri5',{id:'RI#5',icon:'◇',name:'Foundry Control Plane',subtitle:'v0.99 / LF-EKB v0.3 / Runtime Review',state:'ACTIVE',openLabel:'RI#5 Foundry Control Planeの詳細を開く',closeLabel:'RI#5詳細を閉じる',operator:'CONTROL PLANE OPERATOR',image:'ri1-operator.webp',kicker:'RI #5 / FOUNDRY CONTROL PLANE',title:'CAMPAIGN TELEMETRY',comment:'Current Candidate v0.99 / LF-EKB v0.3。Producer RIのEvidence Authorityを保持し、Evidence Explorer Phase 1とHuman InterventionをBindingした。',cards:[['CANDIDATE','v0.99','cyan'],['LF-EKB','v0.3','green'],['EXPLORER','PHASE 1','green'],['RUNTIME VERIFY','PENDING','purple']],pipelineTitle:'EVIDENCE CONTROL',pipeline:[['COLLECT','ok'],['BIND','ok'],['EXPLORE','ok'],['VERIFY','next']],rows:[['Evidence Explorer','PHASE 1'],['Human Input','BOUND'],['Runtime Verification','PENDING']],currentness:[['Candidate','BOUND'],['Authority','PRODUCER RI'],['Acceptance','NOT REACHED']],footerLabel:'CURRENT CANDIDATE',footer:'Evidence ExplorerとHuman InterventionをCurrent CandidateへBinding済み。Human Runtime VerificationとProduction Acceptanceは未完了。'});
    const web = promoteOperationalLine(webLine,'web',{id:'WEB',icon:'▽',name:'公式HP v5.2',subtitle:'Published / RI#4 + RI#5 Current',state:'LIVE',openLabel:'WEB 公式HPの詳細を開く',closeLabel:'WEB詳細を閉じる',operator:'WEBSITE OPERATOR',image:'ri1-operator.webp',kicker:'WEB / HUMAN-FACING PUBLICATION',title:'WEBSITE TELEMETRY',comment:'v5.2を公開中。RI#4 / RI#5 Current CandidateをPublic-safeに反映し、Liveで提供している。',cards:[['VERSION','v5.2','cyan'],['PUBLICATION','PUBLISHED','green'],['PUBLIC CURRENT','RI#4 / RI#5','purple'],['SERVICE','LIVE','green']],pipelineTitle:'PUBLICATION FLOW',pipeline:[['SOURCE','ok'],['PROJECTION','ok'],['GITHUB','ok'],['LIVE','ok']],rows:[['Current Version','v5.2'],['Publication','Published'],['Confirmed Core','FC-CORE-001〜004']],currentness:[['Release','PUBLISHED'],['Presentation','CURRENT'],['Read-Back','VERIFIED']],footerLabel:'PUBLIC CURRENT',footer:'RI#4 v0.64 / RI#5 v0.99 CurrentをWebsite v5.2として公開。'});
  ...
  const currentCopy = qs('#current .current-copy > p:not(.kicker)'); if(currentCopy)currentCopy.textContent='RI#4 v0.64 / RI#5 v0.99をCurrent Candidateとして公開。Evidence Explorer Phase 1を反映し、Human Runtime VerificationとProduction Acceptanceは未完了。';
