...
<button class="project-card active-card" data-open-ri2="" type="button"><span class="project-icon green">⬡</span><span><b><em>RI#2</em> ドキュメント制作</b><small>Document Transformation / Phase B</small></span><mark>READY</mark><i class="card-trace"></i></button>
...
function openPanel({scroll=true,focusPanel=false}={}){
 ...
 board.classList.remove('ri2-open');board.classList.add('ri-open','ri1-open','ri-booting');document.dispatchEvent(new CustomEvent('lf:ri1-opened'));
...
function openPanel({scroll=true,focusPanel=false}={}){
 if(isOpen){if(scroll)scrollPanel();return}
 isOpen=true;clearTimeout(bootTimer);clearTimeout(guideTimer);
 board.classList.remove('ri1-open');board.classList.add('ri-open','ri2-open','ri-booting');
