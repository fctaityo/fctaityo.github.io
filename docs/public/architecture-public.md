...
本書は、Local AI
Foundryの構成、境界、Workflow、主要データフローを公開するためのArchitecture文書である。思想は[基本原則](principles-public.md)、用語は用語集へ委譲する。実装値、内部パス、詳細な運用手順、監査ファイル構成、進行中の実装状況は公開対象外とする。
...
設計思想の正本は[基本原則](principles-public.md)である。本書では、その原則をコンポーネント境界とデータフローへ適用した結果だけを扱う。
...
開発ガバナンスと状態分類の正本はConfiguration Management、採用理由は[ADR-0009](adr/ADR-0009-configuration-management-public.md)とする。本章は構造上の位置付けだけを示し、個別の差分、現在の同期状態、Audit結果は保持しない。

作業の完了条件はDefinition of Done、Codexの標準工程はCodex Standard Operating Procedure、監査履歴はConfiguration Audit一覧へ分離する。
...
[ADR-0006](adr/ADR-0006-artifact-integrity-validation-public.md) と
ADR-0007を参照する。
...
Error Codeの発生条件、Retry可否、Operator対応はError Catalogを正本とする。契約判定、Retry、Agent間受け渡し、保存・画像生成結果は、用途別の監査情報として分離して残す。具体的なファイル名、内部JSON構造、保存先は内部仕様とする。巨大本文を運用ログへ重複出力しない。

打鍵レビュー、実運用レビュー、品質レビューは
[Review Knowledge Base](reviews/README.md) にProject
...
-   DTO契約変更は dify-dto-contracts.md と関連テストを同時更新し、意味変更は必要に応じてADRで記録する
